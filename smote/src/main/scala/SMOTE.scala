...
object SMOTE {
  val log = LogManager.getRootLogger
    ...
    val numObs = dataArray.map(x => x.size).reduce(_+_)
		...
		log.info("Number of Filtered Observations "+numObs.toString)		

		
		val incrementPctg = oversamplingPctg//JMA: remove  + 1
		...
		log.info("Sample Data Count "+sampleData.size.toString)
    ...
    var sampleDataNearestNeighbors = randomNearestNeighbor.zip(sampleData).map(x => (x._1._3._1._1, x._1._2, x._1._3._1._2, x._2._1))
		...
		log.info("New Line Count "+newData.count.toString)
		
		FileUtils.deleteFile(sc, outPath)//JMA
		...
		delimiter: String): Iterator[String]  = {
			...
			for (j <- 0 to sampleDataNNSize){
				...
				if (partitionId == partitionIndex.toInt){
					...
					result.::=("1"+delimiter+sampleFeatures.toArray.mkString(delimiter))	//JMA
				}
			}
		result.iterator
	}		
